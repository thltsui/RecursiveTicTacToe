...
def train(config: TrainingConfig) -> None:
    ...
    total_games = 0
    best_network_state = network.state_dict()
    ...
    try:
        while True:
            ...
            # 1. SELF-PLAY
            network.eval()
            print(f"[Iter {iteration}] Self-play: generating {config.games_per_iteration} games...")
            ...
            for step in tqdm(range(config.batches_per_iteration), desc="Training"):
            ...
            avg_factor = config.batches_per_iteration
            metrics = {
                'iteration': iteration,
            ...
            if iteration % config.arena_every_n == 0:
                try:
                    ...
                    metrics['arena_win_rate'] = result.win_rate
                    print(f"  Arena: {result.wins}W/{result.losses}L/{result.draws}D "
                          f"(win_rate={result.win_rate:.3f})")

                    # 4. CHECKPOINT if new network is better
                    if result.new_is_better:
                        best_network_state = network.state_dict().copy()
                        path = save_checkpoint(network, optimizer, iteration,
                                               0.0, config.checkpoint_dir)
                        print(f"  New best network! Saved to {path}")
                except Exception as e:
                    print(f"  Arena skipped: {e}")
        ...
        path = save_checkpoint(network, optimizer, iteration, 0.0, config.checkpoint_dir)
...
def save_checkpoint(
    ...
    filename = f"checkpoint_iter{iteration:05d}_elo{elo:.0f}.pt"
