...
Because $\gamma < 1$, each application of $T^*$ brings V and U strictly closer together. By the Banach fixed-point theorem, any contraction on a complete metric space has exactly one fixed point — and repeated application of $T^*$ converges to it from any starting point. Without $\gamma < 1$, $T^*$ is no longer a contraction, and there is no guarantee that a unique solution exists or that iterative methods will converge to it.
...
There is a third, deeper reason that we will meet in Essay 1c. The Gittins index theorem — the exact optimal solution to the multi-armed bandit problem under geometric discounting — requires $\gamma < 1$ for the same reason the Bellman equation does. Each arm's optimal value is computed by solving its own Bellman fixed-point equation, which is a contraction only when $\gamma < 1$. More remarkably, the proof that the K-arm optimisation decomposes into K independent single-arm problems depends on the geometric structure of discounting: $\gamma < 1$ makes each arm's future contributions separable from the others'. The discount factor is not just a convergence device — it is the key that unlocks tractability.
...
This is an exponentially weighted running average: each update moves $V($s_t$)$ a fraction $\alpha$ toward the observed return.
...
This quantity $\delta_t$ is the *TD error* — the gap between what we expected ($V($s_t$)$) and what we observed plus what we now expect from the next state (r_t + γ·V($s_{t+1}$)). We update:
...
On the first game, every transition has $r_t$ = 0 and $V($s_{t+1}$)$ = 0. So $\delta_t$ = 0 + γ·0 − 0 = 0. The update is zero. Nothing is learned from any non-terminal state.

For the final winning move from state $s_{T-1}$ into the terminal state $s_T$, the reward is $r_T$ = 1. Since $s_T$ is terminal, it has no future value: V($s_T$) = 0. The TD error for the state just before the end is $\delta_{T-1}$ = 1 + 0 − 0 = 1. Thus, the state *just prior* to winning gets updated: V($s_{T-1}$) moves from 0 to something positive.
...
The fix is to replace the Q-table with a neural network Q(s, a; θ), parameterised by weights $\theta$. Given a state s (encoded as a vector or tensor), the network outputs Q-values for every action simultaneously.
...
An alternative approach is to directly parameterise the policy — the function that maps states to actions. Instead of learning V or Q and deriving a policy from them, we learn $\pi_\theta(a|s)$ directly: a probability distribution over actions given the current state.
...
J($\theta$) is the expected discounted total reward when the agent follows policy π_θ from the start. We want to find $\theta^*$ = argmax_θ J($\theta$).

To maximise J($\theta$), we need its gradient with respect to $\theta$. The policy gradient theorem gives:

$$ \nabla_\theta J(\theta) = \mathbb{E}_{\pi_\theta} \left[ \sum_t \nabla_\theta \log \pi_\theta(a_t \mid s_t) $G_t$ \right] $$

The key object is ∇_θ log π_θ(a_t | s_t) — the gradient of the log-probability of the action taken, with respect to the policy parameters. This is called the *score function*. The intuition: if $G_t$ is large (the game went well), we want to increase the log-probability of the actions we took. If $G_t$ is small, we want to decrease them. The score function tells us which direction to push $\theta$.
...
REINFORCE is the algorithm that implements this directly: play a complete game, compute $G_t$ for every step, and update $\theta$ by gradient ascent. It is Monte Carlo policy gradient — it requires the full return $G_t$, not an estimate.
...
where $\delta_{t+l}$ = r_{t+l} + γ·V(s_{t+l+1}) − V(s_{t+l}) is the TD error at step t+l.

When λ = 0, this collapses to the one-step TD error $\delta_t$. When λ = 1, it becomes the full Monte Carlo return minus the baseline. Any λ ∈ (0,1) interpolates smoothly between the two. GAE is now standard in high-performance RL systems.
