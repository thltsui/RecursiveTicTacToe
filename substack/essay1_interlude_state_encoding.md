...
With the credit-assignment problem understood ([From Zero to AlphaZero: The Reinforcement Learning Landscape](https://tthl.substack.com/p/from-zero-to-alphazero-the-reinforcement)) and our game state perfectly translated into the language of tensors, we are almost ready to build our agent. But first, we need to solve the final piece of the puzzle: how to explore the game tree without getting lost in its immense size.

---

Code: [Notebook 1 — The UTTT Game Engine](https://colab.research.google.com/github/thltsui/RecursiveTicTacToe/blob/Substack/substack/notebook_1_uttt_engine.ipynb) shows the full 7-channel `encode_state()` function in action, including the code that generated the visualizations above.
