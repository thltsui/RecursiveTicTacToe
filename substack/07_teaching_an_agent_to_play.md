...
Here is how a Q-value actually gets learned from played games. Every move's target has two possible parts: the reward earned by that specific move, and, if the game has not ended yet, a discounted estimate of what happens next. The reward part is known exactly the moment the move is played: 0 for a routine move, +0.02 if it wins a sub-board, and, if the move ends the game, +1 for a win, −1 for a loss, or 0 for a draw. Only a game-ending move stops there; every other move's target adds the table's own current best guess for the position it led to, discounted by γ. Replaying each finished game in reverse, last move first, means that guess is often freshly updated within the same game: the table already knows the finishing move's value by the time it computes the target for the move before it.
